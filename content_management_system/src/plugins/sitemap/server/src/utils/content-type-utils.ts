...
export const getContentTypes = (strapi: Core.Strapi): { uid: string; apiName: string }[] => {
...
export const processContentType = (uid: string, contentType: any): SitemapUrl | null => {
  ...
  const isDisplayable =
    contentType.options?.draftAndPublish || contentType.options?.displayable !== false;
  ...
  if (contentType.kind === 'collectionType' && contentType.options?.hasListingPage !== false) {
...
export const getStaticPages = async (strapi: Core.Strapi): Promise<SitemapUrl[]> => {
