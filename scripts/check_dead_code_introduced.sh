...
yarn --silent deadcode > scripts/dead_code_current.txt
...
if [ "$diff_dead_code_count" -lt 0 ]; then
    ...
    display_differences_between_dead_code
    echo '👀 If the increase of the dead code is legit and you tried to update the configuration of ts-prune to ignore it but it does not work, please run `yarn deadcode:update`'
    exit 1
else
